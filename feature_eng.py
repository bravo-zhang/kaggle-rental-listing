...
def designate_single_observations(df1, df2, column):
    ...
    df1.loc[df1.join(grouped, on=column)["size"] <= 1, column] = -1
    df2.loc[df2.join(grouped, on=column)["size"] <= 1, column] = -1
    return df1, df2
...
X_train = X_train.join(pd.get_dummies(X_train["interest_level"], prefix="pred"))
...
columns = ['feature_' + str(s) for s in range(len(mlb.classes_))]
flt = lambda l: [i for i in l if i in mlb.classes_]     # filter out features not present in MultiLabelBinarizer
X_train = X_train.join(pd.DataFrame(data=mlb.transform(X_train["features"].apply(flt)),
                                    columns=columns, index=X_train.index))
X_test = X_test.join(pd.DataFrame(data=mlb.transform(X_test["features"].apply(flt)),
...
X_train.drop([c for c in X_train.columns if c in columns_to_drop], axis=1).\
    to_csv("train_plain_col.csv", index=False, encoding='utf-8')
X_test.drop([c for c in X_test.columns if c in columns_to_drop], axis=1).\
    to_csv("test_plain_col.csv", index=False, encoding='utf-8')
